...
namespace Zenject
{
    ...
    // We'd prefer to make this abstract but Unity 5.3.5 has a bug where references
    // can get lost during compile errors for classes that are abstract
    public class ScriptableObjectOdinInstaller : ScriptableObjectInstallerOdinBase
    {
    }
    ...
    //
    public class ScriptableObjectOdinInstaller<TDerived> : ScriptableObjectOdinInstaller
        where TDerived : ScriptableObjectOdinInstaller<TDerived>
    {
        public static TDerived InstallOdinFromResource(DiContainer container)
        {
            return InstallFromResource(
                ScriptableObjectInstallerUtil.GetDefaultOdinResourcePath<TDerived>(), container);
        }

        public static TDerived InstallFromResource(string resourcePath, DiContainer container)
        {
            var installer = ScriptableObjectInstallerUtil.CreateOdinInstaller<TDerived>(resourcePath, container);
    ...
    public static class ScriptableObjectInstallerUtil
    {
        ...
        public static string GetDefaultOdinResourcePath<TInstaller>()
            where TInstaller : ScriptableObjectInstallerOdinBase
        {
            return "Installers/" + typeof(TInstaller).PrettyName();
        }
        ...
        public static TInstaller CreateOdinInstaller<TInstaller>(
            string resourcePath, DiContainer container)
            where TInstaller : ScriptableObjectInstallerOdinBase
        {
            var installers = Resources.LoadAll(resourcePath);
